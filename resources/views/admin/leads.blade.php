...
@section('content')
    ...
    @if($page == "all_leads")
        <div class="container-fluid pt-4 px-4">
            <div class="py-3"><h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1></div>
            @include('admin.partial.datatable')
        </div>
    @endif

    @if($page == "powur_qualify_leads")
        <div class="container-fluid pt-4 px-4">
            <div class="py-3"><h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1></div>
    ...
    @endif
    @if($page == "truenergy_qualify_leads")
        <div class="container-fluid pt-4 px-4">
            <div class="py-3"><h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1></div>
            @include('admin.partial.datatable')
        </div>
    @endif
    @if($page == "all_qualify_leads")
        <div class="container-fluid pt-4 px-4">
            <div class="py-3"><h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1></div>
            @include('admin.partial.datatable')
        </div>
    @endif
    @if($page == "unqualify_leads")
        <div class="container-fluid pt-4 px-4">
            <div class="py-3">
                <h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1>
            </div>
            @include('admin.partial.datatable')
        </div>
    @endif
    @if($page == "incomplete_leads")
        <div class="container-fluid pt-4 px-4">

            <div class="py-3">
                <h1 class="text-center pb-2" style="border-bottom: 2px solid"> {{$title}}</h1>
                <input type="button" onclick="add_to_GR();" class="send_gr_btn"
                       value="Send {{$incomplete_count}} Data To GR">
            </div>
            @include('admin.partial.datatable')
        </div>
    @endif
...
@section('footer_scripts')
 ...
 //        }).toString().match(/[^]*\/\*([^]*)\*\/\}$/)[1];
        ...
        // Ajax Request For Adding data of Incomplete leads to GR
        function add_to_GR() {
            let data = new FormData();
            data.append('_token', "{{csrf_token()}}");
            let request = new XMLHttpRequest();
            request.open("POST", '{{url('admin/incomplete_to_gr')}}', true);
            request.send(data);
            $('body').append(modal);
